...
import { getWebhookSecret, verifyWebhookSignature } from '@/lib/github/webhook';
...
export async function POST(request: NextRequest) {
	try {
		...
		// 7. 忽略来自 Github Translator 的提交（避免循环触发）
		if (head_commit?.message.startsWith('[Github Translator]')) {
			console.log('[Webhook] Ignoring commit from Github Translator');
