...
| 字段 | 值 |
|------|-----|
| **GitHub App name** | `Github Translator` (或你自定义的名称) |
...
-- 创建数据库
CREATE DATABASE github_translator CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 创建用户（可选）
CREATE USER 'github_translator'@'localhost' IDENTIFIED BY 'your_password';
GRANT ALL PRIVILEGES ON github_translator.* TO 'github_translator'@'localhost';
FLUSH PRIVILEGES;
```
...
```bash
# 数据库配置
DATABASE_URL="mysql://github_translator:your_password@localhost:3306/github_translator"
